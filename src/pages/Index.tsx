...
import { roomService, type Room, type AvailabilityResponse, RoomAvailability } from "@/services/room.service";
          ...
          <div className="flex justify-center gap-8 mb-8">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-accent">
                {rooms?.reduce((acc, room) => acc + (room.tables?.length || 0), 0) || 0}
              </div>
  ...
  <CardContent>
    <div className="grid md:grid-cols-3 gap-4">
      {rooms?.map((room) => {
        ...
        return (
          <div key={room._id} className="flex flex-col p-3 rounded-lg bg-muted/30 border">
            {/* Room Header with Availability */}
            <div className="flex justify-between items-center mb-2">
              <span className="font-medium">{room.name}</span>
              {roomAvailability ? (
                <span className={`text-xs px-2 py-1 rounded-full ${
                  roomAvailability.availabilityPercentage >= 70 ? 'bg-green-100 text-green-800' : 
                  roomAvailability.availabilityPercentage >= 40 ? 'bg-yellow-100 text-yellow-800' : 
            ...
            <div className="grid grid-cols-2 gap-2 text-sm mb-3">
              <div className="space-y-1">
                ...
                <div>
                  {roomAvailability ? (
                    <span>
                      <span className="font-medium">{roomAvailability.availableTables}</span>
                      <span className="text-muted-foreground">/{roomAvailability.totalTables}</span>
                    </span>
                  ) : (
